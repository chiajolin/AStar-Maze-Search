...
public class SmallerGForwardAStar implements Comparator<Cell> {
	...
	public int F(Cell x) {
		...
		return x.ManhattanDistance(targetCell) + x.G;
	}

	public SmallerGForwardAStar() {
		...
		while (startCell!= targetCell) {
			...
			while(st.isEmpty()==false)
			{
				...
				if(temp!=targetCell){temp.setCellType("*");}stepcount++;
				startCell=temp;
	...
	public int compare(Cell a, Cell b) {
		if (F(a) == F(b)) {
			if (a.G == b.G) {
				...
				return a.G-b.G;
			} // prioritize smaller G if F are equal
		} else {
			return F(a) - F(b);
